...
for (i in seq_len(N_rounds)){
  ...
  for (p in seq_len(length(n_clusters))){
    
    print(glue("\n--- round {i} starting ---\n"))
    
    sample <- sample_data(pbmcData, 
    ...
    targets <- assay[out$genesymbols[c(which(out$is_regulator==0))],]
    
    targets <- targets
    
    clusters <- scregclust::kmeanspp(targets, 
                                     n_cluster = n_clusters[p],
                                     n_init_clusterings = 10,
                                     n_max_iter = 10L)
    
    clusters_km <- kmeans(targets,
                          centers=n_clusters[p],
                          iter.max = 10L, 
                          nstart = 10)$cluster
    
    #kmpp <- motifcluster::kmeanspp
    #clusters <- kmpp(assay, 
    #                                   k = n_clusters[p],
    #                                   iter.max = 10L,
    #                                   nstart = 10)$cluster
    #print(out$genesymbols[1:5])
    #print(clusters[1:5])
    
    times <- dim(targets)[1]
    
    index_targets <- match(out$genesymbols, row.names(targets))

    index_targets <- index_targets[!is.na(index_targets)]
    
    
    df_temp <- data.frame(
                            iteration = c(rep(i, times = times)),
                            n_clusters = c(rep(n_clusters[p], times = times)),
                            gene = c(row.names(targets)),
                            is_regulator = c(out$is_regulator[index_targets]),
                            cluster = c(clusters),
                            cluster_km = c(clusters_km)
                            )
      ...
      rm(df_temp)
      #rm(kmpp)
      print(glue("\n--- n_clus {n_clusters[p]} done ---\n"))

    
  }
  
  print(glue("\n--- round {i} done ---\n"))
  write.csv(df, glue("benchmark_kmeanspp/test_iterations_v3/iteration{i}_hh.csv"), row.names=FALSE)
  rm(df)
  rm(assay)
  rm(out)
  rm(sample)
...
```{r}
length(head(match(out$genesymbols, row.names(targets))))
```
...
folder_path <- 'benchmark_models'
file <- 'v1_sample_11_penalty0.1_fit.rds'

fit_test = readRDS(glue('{folder_path}/{file}'))


fit_test$results[[1]]$output[[1]]$cluster_all[[which(fit_test$results[[1]]$genesymbols=='LUC7L')]]



is_regulator <- fit_test$results[[1]]$is_regulator

genesymbols <- fit_test$results[[1]]$genesymbols
...
```{r}
genesymbols[is_regulator == 1][fit_test$results[[1]]$output[[1]]$models[, 7]]
```
