...
for (i in seq_len(N_rounds)){
  ...
  for (p in seq_len(length(n_clusters))){
    ...
    #clusters <- scregclust::kmeanspp(assay, 
    #                                 n_cluster = n_clusters[p],
    #                                 n_init_clusterings = 10,
    #                                 n_max_iter = 10L)
    
    clusters <- motifcluster::kmeanspp(assay, 
                                       k = n_clusters[p],
                                       iter.max = 10L,
                                       nstart = 10)$cluster
    ...
    df_temp <- data.frame(
                            ...
                            gene = c(out$genesymbols),
                            is_regulator = c(out$is_regulator),
                            cluster = c(clusters)
                            )
  ...
  print(glue("\n--- round {i} done ---\n"))
  write.csv(df, glue("benchmark_kmeanspp/test_iterations_v2/iteration{i}.csv"), row.names=FALSE)
  rm(df)
  rm(assay)
  rm(out)
  rm(sample)
